...
def run_gpt_prompt_task_decomp(persona, 
  ...
  put_in_db(db, persona.scratch.name, step,
            ...
            "gpt-3.5-turbo-instruct", 1000, str(prompt), str(prompt))
...
def run_gpt_prompt_action_sector(action_description, 
  ...
  put_in_db(db, persona.scratch.name, step,
            ...
            "gpt-3.5-turbo-instruct", 15, str(prompt), str(output))
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False,db = None,step = None): 
  ...
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = "🛁🧖‍♀️" ########
  example_output = f"{{\"output\": 🛁🧖‍♀️}}"
  # special_instruction = "The value for the output must ONLY contain the emojis." ########
  special_instruction = "only return the emoji with the output format of {\"output\": <output>}. Do not generate code"
  fail_safe = get_fail_safe()
  start = time.time()
  # output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
  #                                         __chat_func_validate, __chat_func_clean_up, True)
  output = llm_safe_generation(prompt,example_output,special_instruction,3,fail_safe, 
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False, db = None, step = None): 
  ...
  put_in_db(db, persona.scratch.name, step,
            ...
            "gpt-3.5-turbo-instruct", 30, str(prompt), str(output))
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False, db = None, step = None): 
  ...
  example_output = f"{{output: {act_game_object} is idle}}"
  ...
  #                                         __chat_func_validate, __chat_func_clean_up, True)
  output = llm_safe_generation(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, gpt_param)
  end = time.time()
  put_in_db(db, persona.scratch.name, step,
...
def run_gpt_prompt_act_obj_event_triple(act_game_object, act_obj_desc, persona, verbose=False, db = None, step = None): 
  ...
  put_in_db(db, persona.scratch.name, step,
            ...
            "gpt-3.5-turbo-instruct", 30, prompt, str(output))
...
def run_gpt_prompt_new_decomp_schedule(persona, 
  ...
  put_in_db(db, persona.scratch.name, step,
            ...
            "gpt-3.5-turbo-instruct", 1000, str(prompt), str(output))
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved,test_input=None, 
  ...
  put_in_db(db, persona.scratch.name, step,
            ...
            "gpt-3.5-turbo-instruct", 20, str(prompt), str(output))
...
def run_gpt_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False,db = None, step = 0): 
  ...
  example_output = f"{{\"output\": 5}}" ########
  special_instruction = "The output should ONLY contain ONE integer value on the scale of 1 to 10. Generate results in json format like {\"output\": <output>}. Do not generate code"
  fail_safe = get_fail_safe() ########
  # output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
  #                                         __chat_func_validate, __chat_func_clean_up, True)
  output = llm_safe_generation(prompt, example_output, special_instruction, 3, fail_safe,
  ...
  if db is not None:
    put_in_db(db, persona.scratch.name, step, "run_gpt_prompt_event_poignancy",
              ...
              max_token= 15, input = str(prompt), output = output
              )
  if output != False: 
    if debug or verbose: 
      print_run_prompts(prompt_template, persona, gpt_param, 
...
def run_gpt_prompt_agent_chat_summarize_relationship(persona, target_persona, statements, test_input=None, verbose=False,db = None, step = None): 
  ...
  gpt_param = { "max_tokens": 50,"temperature":0.3,"frequency_penalty": 0.1}
  prompt_template = "persona/prompt_template/v3_ChatGPT/summarize_chat_relationship_v2.txt" ########
  prompt_input = create_prompt_input(persona, target_persona, statements)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = '{\"output\": Jane Doe is working on a project}' ########
